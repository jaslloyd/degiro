...
const create = ({
  ...
  const getAskBidPrice = (issueId, timesChecked = 0) =>
    requestVwdSession().then((vwdSession) => {
      const checkData = (res) => {
        ...
        if (res.length == 1 && res[0].m == "h") {
          if (timesChecked <= 3) {
            ...
            throw Error(
              "Tried 3 times to get data, but nothing was returned: " +
                JSON.stringify(res)
            );
          }
        }
        ...
        if (
          ...
          throw Error(
            "Couldn't find all requested info: " + JSON.stringify(res)
          );
        }
      ...
      return fetch(
        `https://degiro.quotecast.vwdservices.com/CORS/${vwdSession.sessionId}`,
        {
          method: "POST",
          headers: { Origin: "https://trader.degiro.nl" },
          body: JSON.stringify({
   ...
   */
  const getOrders = () => {
    return getData({ orders: 0, historicalOrders: 0, transactions: 0 }).then(
      (data) => {
        if (
          data.orders &&
